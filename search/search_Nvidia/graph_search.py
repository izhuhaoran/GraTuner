...
def search(knn_graph, k, graph_id, algo_name, n, schedules_origin, schedules_data, model):
    ...
    logger = get_logger(f"search_{algo_name}_{graph_name}", f"{PROJECT_ROOT}/search/search_GPU/search_{algo_name}_{graph_name}.log")
...
def search_v2(knn_graph, k, graph_id, algo_name, n, schedules_origin, schedules_data, model):
    graph_name = graph_list[graph_id]
    logger = get_logger(f"search_v2_{algo_name}_{graph_name}", f"{PROJECT_ROOT}/search/search_GPU/search_v2_{algo_name}_{graph_name}.log")
...
def search_v2_print_iter(knn_graph, k, graph_id, algo_name, n, schedules_origin, schedules_data, model):
    graph_name = graph_list[graph_id]
    logger = get_logger(f"search_v2_print_iter_{algo_name}_{graph_name}_{k}", f"{PROJECT_ROOT}/search/search_GPU/search_diff_k_iter_{algo_name}_{graph_name}_k{k}.log")
...
@torch.no_grad()
def search_v3_print_iter(knn_graph, k, graph_id, algo_name, n, schedules_origin, schedules_data, model, top_m=1):
    graph_name = graph_list[graph_id]
    logger = get_logger(f"search_v3_print_iter_{algo_name}_{graph_name}_{top_m}", f"{PROJECT_ROOT}/search/search_GPU/search_diff_topm_iter_{algo_name}_{graph_name}_top_m{top_m}.log")
    ...
    for _ in range(5):
        for algo_name in algo_list:
            ...
            for graph_id in range(len(graph_list)):
                knn_graph_path = f"{PROJECT_ROOT}/search/search_GPU/trained_graph/one_graph/knn_graph_best_{algo_name}_{graph_list[graph_id]}.pkl"

...
from data_loader import ScheduleDataset, GraphsDataset, ScheduleGraphDataset, ScheduleDataset, ScheduleDataset_Onehot
from model import AutoGraphModel_NoAlgo, AutoGraphModel
import logging
from common.common import PROJECT_ROOT

# Configure logging
# logging.basicConfig(filename='{PROJECT_ROOT}/cost_model/cost_model_Intel/train_with_algo/trainlog_gcn_transformer.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
logger = get_logger("train_with_algo", "{PROJECT_ROOT}/cost_model/cost_model_Intel/train_with_algo/trainlog_gcn_transformer.log")
...
def train_onehot():
    algo_name = 'pagerank'
    train_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/train/{algo_name}.csv'
    val_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/val/{algo_name}.csv'
    ...
    # checkpoint = torch.load("{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result/costmodel_290.pth")
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), "{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result/costmodel_best.pth")
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result/costmodel_{epoch+300}.pth")
...
def train_all_algo_v2():
    ...
    # checkpoint = torch.load("{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result/costmodel_best_transformer.pth")
    # model.load_state_dict(checkpoint)
    ...
    for epoch in range(epoches):
        ...
        for algo in ["pagerank", "sssp", "bfs", "cc",]:
            ...
            train_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/train/{algo}.csv'
            val_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/val/{algo}.csv'
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), "{PROJECT_ROOT}/cost_model/cost_model_Intel/train_with_algo/costmodel_best.pth")
            test_accuracy_best = test_accuracy
            test_best_epoch = epoch
            logger.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_Intel/train_with_algo/costmodel_{epoch}.pth")

...
from cost_model.cost_model_Nvidia.data_loader import kernel_fusion_map, LB_map, EB_map, direction_map, dedup_map, frontier_output_map, pull_rep_map
...
def read_all_schedules(path):
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            sche_tmp[0] = kernel_fusion_map[processed[0]]
            sche_tmp[1] = LB_map[processed[1]]

            sche_tmp[2] = direction_map[processed[4]]
            sche_tmp[3] = dedup_map[processed[5]]
            sche_tmp[4] = frontier_output_map[processed[6]]
            sche_tmp[5] = pull_rep_map[processed[7]]
...
def knn_graph_create(model, schedules_data, n, m=10, random_init=False, all_graph_stack=False, out_base_name='knn_graph_init'):
    ...
    if all_graph_stack:
        if random_init:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_GPU/init_graph/{out_base_name}_all_graph_random.pkl")
        else:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_GPU/init_graph/{out_base_name}_all_graph.pkl")
    else:
        if random_init:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_GPU/init_graph/{out_base_name}_init_random.pkl")
        else:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_GPU/init_graph/{out_base_name}.pkl")    
...
if __name__ == "__main__":
    ...
    for algo in algo_list:
        schedules_origin, schedules_data = read_all_schedules(f"{PROJECT_ROOT}/search/search_GPU/all_true_schedule_{algo}_GPU.csv")

...
# Configure logging
logging.basicConfig(filename='/home/zhuhaoran/AutoGraph/AutoGraph/cost_model/train_result_nograph/trainlog.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
def train_onehot():
    ...
    # checkpoint = torch.load("/home/zhuhaoran/AutoGraph/AutoGraph/cost_model/costmodel_gat_best.pth")
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            ...
            logging.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))

        torch.save(model.state_dict(), f"/home/zhuhaoran/AutoGraph/AutoGraph/cost_model/train_result/costmodel_{epoch}.pth")
...
def train_no_graph():
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), "/home/zhuhaoran/AutoGraph/AutoGraph/cost_model/train_result_nograph/costmodel_best.pth")
            test_accuracy_best = test_accuracy
            test_best_epoch = epoch
            logging.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))

        torch.save(model.state_dict(), f"/home/zhuhaoran/AutoGraph/AutoGraph/cost_model/train_result_nograph/costmodel_{epoch}.pth")

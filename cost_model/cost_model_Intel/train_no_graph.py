...
from model import AutoGraphModel_NoAlgo, AutoGraphModel_GAT, AutoGraphModel_OnlySche
import logging
from common.common import PROJECT_ROOT

# Configure logging
# logging.basicConfig(filename=f'{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result_nograph/trainlog_no_graph_finetune.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
dataset_folder = '{PROJECT_ROOT}/dataset/dataset_Intel/finetune'
...
logger = get_logger("train_no_graph", "{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result_nograph/trainlog_no_graph_finetune.log")
...
def train_onehot():
    ...
    # checkpoint = torch.load("{PROJECT_ROOT}/cost_model/cost_model_Intel/costmodel_gat_best.pth")
...
def train_no_graph():
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result_nograph/costmodel_best_{algo_name}.pth")
            test_accuracy_best = test_accuracy
            test_best_epoch = epoch
            logger.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))

        # torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result_nograph/costmodel_{epoch}.pth")
...
def train_all_algo_no_graph():
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), "{PROJECT_ROOT}/cost_model/cost_model_Intel/train_result_nograph/costmodel_best.pth")
            test_accuracy_best = test_accuracy
            test_best_epoch = epoch
            logger.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))

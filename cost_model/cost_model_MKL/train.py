...
# Configure logging
# logging.basicConfig(filename=f'{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result/trainlog_gcn_transformer_finetune.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
# logger = get_logger("train_logger", f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result/trainlog_gcn_transformer_finetune.log")
...
def train_all_algo_only_data():
    logger = get_logger("train_all_algo_only_data_logger", f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result/trainlog_gcn_transformer_finetune.log")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = AutoGraphModel()
    
    # checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result/costmodel_best.pth")
    # model.load_state_dict(checkpoint)
    ...
    for epoch in range(epoches):
        ...
        # torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result/costmodel_latest.pth")


def train_all_algo_only_data_mkl():
    logger = get_logger("train_all_algo_only_data_mkl_logger", f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result/trainlog_gcn_transformer_finetune_mkl.log")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = AutoGraphModel().to(device)

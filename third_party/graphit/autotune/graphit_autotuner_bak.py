...
class GraphItTuner(MeasurementInterface):
    ...
    def compile_and_run(self, desired_result, input, limit):
        ...
        if not self.schedule_fliter(cfg):
            with open('{PROJECT_ROOT}/graphit/autotune/output_autotune/cfg_no_pass.txt', '+a') as fw_cfg:
        ...
        with open('{PROJECT_ROOT}/graphit/autotune/output_autotune/cfg.txt', '+a') as fw_cfg:
            fw_cfg.write(str(cfg)+'\n')
        
        with open('{PROJECT_ROOT}/graphit/autotune/output_autotune/time_log_iter.txt', '+a') as fw_time:
            fw_time.write(f"write_cfg_time: {t2 - t1}, compile_time: {t3 - t2}, run_time: {t4 - t3}, exec_time: {exec_time_once}\n")
        
        with open('{PROJECT_ROOT}/graphit/autotune/output_autotune/time_log_all.txt', '+a') as fw_time:
...
if __name__ == '__main__':
    ...
    all_time = t2 - time_0
    with open('{PROJECT_ROOT}/graphit/autotune/output_autotune/time_log.txt', '+a') as fw_time:
        fw_time.write(f"all_time: {all_time}, write_cfg_time: {write_cfg_time}, compile_time: {compile_time}, run_time: {run_time}, exec_time: {exec_time}\n")

...
from data_loader import ScheduleDataset, ScheduleDataset_Onehot, ScheduleDataset_all_algo
...
logger = get_logger("train_logger", "{PROJECT_ROOT}/cost_model/cost_model_Intel/finetune_result/trainlog_gcn_transformer_algo.log")
...
def train_algo(algo_name='pagerank'):
    ...
    train_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/finetune/train/{algo_name}.csv'
    val_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/finetune/val/{algo_name}.csv'
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), "{PROJECT_ROOT}/cost_model/cost_model_Intel/finetune_result/costmodel_best.pth")
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_Intel/finetune_result/costmodel_{epoch+300}.pth")



def train_all_algo_bak():
    
    train_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/finetune/train/all_algo.csv'
    val_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/finetune/val/all_algo.csv'
...
def run_batch(model, batch_data, g, algo, device, criterion, optimizer=None, is_train=True):
    """将每个batch的训练/测试逻辑封装,方便管理内存"""
    schedule, runtime = batch_data
    schedule = schedule.to(device)
    runtime = runtime.to(device)
    ...
    # 清理中间变量
    del graph_feature, predict, pred1, pred2, true1, true2, sign, pred_sign
    torch.cuda.empty_cache()
    
    return loss.item(), equal_count, unequal_count
...
def train_all_algo_v2():
    ...
    for epoch in range(epoches):
        ...
        for algo in ["pagerank", "sssp", "bfs", "cc",]:
            ...
            train_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/finetune/train/{algo}.csv'
            val_file_path = f'{PROJECT_ROOT}/dataset/dataset_Intel/finetune/val/{algo}.csv'
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_Intel/finetune_result/costmodel_latest.pth")

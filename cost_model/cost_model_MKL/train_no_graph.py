...
from data_loader import ScheduleDataset, GraphsDataset, ScheduleGraphDataset, ScheduleDataset_v2, ScheduleDataset_Onehot, ScheduleDataset_all_algo
from model import AutoGraphModel, AutoGraphModel_GAT, AutoGraphModel_NoGraph
import logging
from common.common import PROJECT_ROOT

# Configure logging
# logging.basicConfig(filename=f'{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/trainlog_no_graph_finetune.log', level=logging.INFO, format='%(asctime)s %(message)s')

lr = 1e-4
batch_size = 32
epoches = 200

# dataset_folder = f'{PROJECT_ROOT}/dataset'
dataset_folder = f'{PROJECT_ROOT}/dataset/dataset_MKL/finetune'
...
logger = get_logger("train_logger", f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/trainlog_no_graph_finetune.log")
...
def load_data_all_algo(file_path):
    origin_data = pd.read_csv(file_path, comment='#', sep=',', header=None)
    return origin_data.values

def train_onehot():
    algo_name = 'pagerank'
    train_file_path = f'{dataset_folder}/train/{algo_name}.csv'
    val_file_path = f'{dataset_folder}/val/{algo_name}.csv'
    ...
    # checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_MKL/costmodel_gat_best.pth")
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/costmodel_best.pth")
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/costmodel_{epoch}.pth")
    ...
    for epoch in range(epoches):
        ...
        for graph_name in graph_list:
            ...
            for train_batchidx, (schedule, runtime) in enumerate(train_data_schedule):
                ...
                # 前向传播
                predict = model.forward(schedule)
                if predict.shape[0] < 2:
                    continue
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/costmodel_best_{algo_name}.pth")
            test_accuracy_best = test_accuracy
            test_best_epoch = epoch
            logger.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))
...
def train_all_algo_no_graph():
    logger = get_logger("train_all_algo_no_graph_logger", f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/trainlog_no_graph_finetune.log")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = AutoGraphModel_NoGraph()
    
    # checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/costmodel_best_transformer.pth")
    # model.load_state_dict(checkpoint)
    ...
    for epoch in range(epoches):
        ...
        for algo in ["pagerank", "sssp", "bfs", "cc",]:
            ...
            train_file_path = f'{dataset_folder}/train/{algo}_gather.csv'
            val_file_path = f'{dataset_folder}/val/{algo}_gather.csv'
...
def train_all_algo_no_graph_mkl():
    logger = get_logger("train_all_algo_no_graph_mkl_logger", f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/trainlog_no_graph_finetune_mkl.log")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    model = AutoGraphModel_NoGraph().to(device)
    ...
    for epoch in range(epoches):
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_MKL/train_result_nograph/costmodel_latest.pth")


if __name__ == '__main__':
    # train_onehot()
    # train_no_graph()
    # train_all_algo_no_graph()
    train_all_algo_no_graph_mkl()

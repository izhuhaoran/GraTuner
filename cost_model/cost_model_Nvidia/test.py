                ...
                for parallelization_option in config_parallelization:
                    ...
                    for DenseVertexSet_option in config_DenseVertexSet:
                        ...
                        if passed: # 如果能够通过过滤，则调度组合有效
                            ...
                            output_file_name = f'{PROJECT_ROOT}/dataset/dataset_GPU/all_true_schedule.csv'
...
if __name__ == '__main__':
    ...
    # test_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/all_true_schedule.csv'
    # test_all_data = pd.read_csv(test_file_path, comment='#', sep=',', header=None)
    # test_all_data = test_all_data.values

    graph_name = 'dblp-cite'
    
    test_all_data = load_data(f'{PROJECT_ROOT}/dataset/dataset_GPU/train/pagerank.csv')
    ...
    checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_GPU/costmodel_v2_best.pth")

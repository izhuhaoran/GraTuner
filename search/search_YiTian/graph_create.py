...
from cost_model.cost_model_YiTian.data_loader import direction_map, parallelization_map, DenseVertexSet_map, SSGNum_map
...
def knn_graph_create(model, schedules_data, n, m=10, random_init=False, all_graph_stack=False, out_base_name='knn_graph_init'):
    ...
    if all_graph_stack:
        if random_init:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_YiTian/init_graph/{out_base_name}_all_graph_random.pkl")
        else:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_YiTian/init_graph/{out_base_name}_all_graph.pkl")
    else:
        if random_init:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_YiTian/init_graph/{out_base_name}_init_random.pkl")
        else:
            save_knn_graph(knn_graph, f"{PROJECT_ROOT}/search/search_YiTian/init_graph/{out_base_name}.pkl")    
...
if __name__ == "__main__":
    ...
    for algo in algo_list:
        schedules_origin, schedules_data = read_all_schedules(f"{PROJECT_ROOT}/search/search_YiTian/all_true_schedule_{algo}_YiTian.csv")

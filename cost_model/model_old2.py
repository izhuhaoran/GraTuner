...
class AutoGraphModel_Onehot(nn.Module):
    def __init__(self, in_dim=2, hidden_dim=256, out_dim=128) -> None:
        super(AutoGraphModel_Onehot, self).__init__()
    ...
    def embed_graph(self, g : DGLGraph) :
        g = dgl.add_self_loop(g)
        # 我们用节点的度作为初始节点特征。
...
class AutoGraphModel_GIN(nn.Module):
    def __init__(self, in_dim=2, hidden_dim=256, out_dim=128) -> None:
        super(AutoGraphModel_GIN, self).__init__()
        
        # Algorithm feature embedding
        self.algo_op1 = nn.Embedding(5, 64)
        self.algo_op2 = nn.Embedding(5, 64)
        self.algorithm_embedding = nn.Sequential(
            nn.Linear(64 * 2, 128),
            nn.ReLU(),
            nn.Linear(128, 128),
        ...
        # Graph feature embedding with GIN
        self.gin_layer1 = GINConv(
            nn.Sequential(
        ...
        self.graph_fc = nn.Linear(hidden_dim, out_dim)

        # Schedule feature embedding
        self.schedule_embedding = nn.Sequential(
            nn.Linear(16, 128),
            nn.ReLU(),
            nn.Linear(128, 128),
        ...
        # Attention layers
        self.attention = nn.MultiheadAttention(embed_dim=256, num_heads=4, dropout=0.3)
    ...
    def embed_graph(self, g: dgl.DGLGraph):
        g = dgl.add_self_loop(g)
        
        # 初始节点特征：度特征
        h1 = g.in_degrees().view(-1, 1).float()
        h2 = g.out_degrees().view(-1, 1).float()
        h = torch.cat([h1, h2], dim=1)  # [N, in_dim=2]
        
        # GIN 层
        h = F.relu(self.gin_layer1(g, h))
        h = F.relu(self.gin_layer2(g, h))
        
        # 节点特征聚合为图特征
        g.ndata['h'] = h
        hg = dgl.mean_nodes(g, 'h')  # 平均池化为图特征
        return self.graph_fc(hg)
    
    def embed_schedule(self, schedule):
        return self.schedule_embedding(schedule)

    def forward_after_query(self, x, schedule):
        schedule_feature = self.embed_schedule(schedule)
        xy = torch.cat((x, schedule_feature), dim=1)
        xy = self.final(xy)
        return xy

    def forward(self, graph, schedule):
        # 图特征编码
        graph_feature = self.embed_graph(graph)  # [batch, 128]
        # 调度特征编码
        schedule_feature = self.embed_schedule(schedule)  # [batch, 128]
        
        # 图特征和调度特征融合
        combined = torch.cat([graph_feature, schedule_feature], dim=1)  # [batch, 256]
        combined, _ = self.attention(combined.unsqueeze(0), combined.unsqueeze(0), combined.unsqueeze(0))  # [1, batch, 256]
        combined = combined.squeeze(0)  # [batch, 256]
        
        # 最终预测
        xy = self.final(combined)
        return xy

...
# Configure logging
logging.basicConfig(filename='/home/zhuhaoran/AutoGraph/AutoGraph/cost_model/train_result/trainlog_gcn_transformer.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
def train_onehot():
    ...
    checkpoint = torch.load("/home/zhuhaoran/AutoGraph/AutoGraph/cost_model/train_result/costmodel_290.pth")
    ...
    for graph_name in graph_list:
        ...
        with torch.no_grad():
            for val_batchidx, (schedule, runtime) in enumerate(val_data_schedule):
                ...
                accuracy_test = equal_count / (equal_count + unequal_count)
                kendall_tau_test = (equal_count - unequal_count) / (equal_count + unequal_count)
                logging.info("--- Test batch ---  : graph {} Test Batch {} Loss {} Accuracy {} Kendall {} ---".format(graph_name, val_batchidx, loss.item(), accuracy_test, kendall_tau_test))
        ...
        accuracy_graph_test = rank_equal_num_graph_test / rank_all_num_graph_test
        kendall_tau_graph_test = (rank_equal_num_graph_test - rank_unequal_num_graph_test) / rank_all_num_graph_test
        logging.info("--- Test graph --- : graph {} Test Acc {} Kendall {} ---".format(graph_name, accuracy_graph_test, kendall_tau_graph_test))

    test_accuracy = rank_equal_num_test / rank_all_num_test
    test_kendall = (rank_equal_num_test - rank_unequal_num_test) / rank_all_num_test

    logging.info("--- Valid {} Test Acc {} Test Kendall {} ---".format(valid_loss / valid_loss_cnt, test_accuracy, test_kendall))

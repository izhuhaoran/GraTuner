...
gpu_runtime::GraphT<int32_t> __device__ edges__transposed;
gpu_runtime::GraphT<int32_t> __host_edges__transposed;
...
bool __device__ toFilter(int32_t v);
...
void __device__ gpu_operator_body_2(gpu_runtime::GraphT<EdgeWeightType> graph, int32_t src, int32_t dst, int32_t edge_id, gpu_runtime::VertexFrontier input_frontier, gpu_runtime::VertexFrontier output_frontier) {
	...
	if (!gpu_runtime::checkBit(input_frontier.d_bit_map_input, dst))
		return;
	EdgeWeightType weight = graph.d_edge_weight[edge_id];
	updateEdge(dst, src, weight, output_frontier);
}
void __device__ SP_generated_vector_op_apply_func_0(int32_t v) {
	SP[v] = 2147483647;
}
...
int __host__ main(int argc, char* argv[]) {
	...
	for (int32_t trail = 0; trail < 10; trail++) {
		...
		__host_SP[start_vertex] = start_vertex;
		...
		while ((gpu_runtime::builtin_getVertexSetSize(frontier)) != (0)) {
			gpu_runtime::VertexFrontier output;
			{
				...
				output = frontier;
				gpu_runtime::TWC_load_balance_host<int32_t, gpu_operator_body_2, gpu_runtime::AccessorSparse, toFilter>(__host_edges__transposed, frontier, output);

...
# Configure logging
# logging.basicConfig(filename=f'{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/trainlog_gcn_transformer_finetune.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
dataset_folder = f'{PROJECT_ROOT}/dataset/dataset_YiTian/finetune'
...
logger = get_logger("train_logger", f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/trainlog_gcn_transformer_finetune.log")
...
def train_onehot():
    algo_name = 'pagerank'
    train_file_path = f'{PROJECT_ROOT}/dataset/dataset_YiTian/train/{algo_name}.csv'
    val_file_path = f'{PROJECT_ROOT}/dataset/dataset_YiTian/val/{algo_name}.csv'
    ...
    # checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/costmodel_290.pth")
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/costmodel_best.pth")
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/costmodel_{epoch+300}.pth")
...
def train_all_algo_only_data():
    ...
    checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/costmodel_best.pth")
    model.load_state_dict(checkpoint)
    ...
    for epoch in range(epoches):
        ...
        # torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/costmodel_{epoch}.pth")
...
#     # checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_result/costmodel_best_transformer.pth")
#     # model.load_state_dict(checkpoint)
...
#             train_file_path = f'{PROJECT_ROOT}/dataset/dataset_YiTian/train/{algo}.csv'
#             val_file_path = f'{PROJECT_ROOT}/dataset/dataset_YiTian/val/{algo}.csv'
...
#         if test_accuracy > test_accuracy_best:
#             torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_with_algo/costmodel_best.pth")
#             test_accuracy_best = test_accuracy
#             test_best_epoch = epoch
#             logger.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))
...
#         torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_YiTian/train_with_algo/costmodel_{epoch}.pth")
...
def test():
    ...
    rank_all_num_test = 0
    for graph_name in graph_list:
        print(f"test: Graph {graph_name} start")
        # torch.cuda.empty_cache()
        ...
        with torch.no_grad():
            for val_batchidx, (schedule, runtime) in enumerate(val_data_schedule):
                ...
                accuracy_test = equal_count / (equal_count + unequal_count)
                kendall_tau_test = (equal_count - unequal_count) / (equal_count + unequal_count)
                logger.info("--- Test batch ---  : graph {} Test Batch {} Loss {} Accuracy {} Kendall {} ---".format(graph_name, val_batchidx, loss.item(), accuracy_test, kendall_tau_test))
        ...
        accuracy_graph_test = rank_equal_num_graph_test / rank_all_num_graph_test
        kendall_tau_graph_test = (rank_equal_num_graph_test - rank_unequal_num_graph_test) / rank_all_num_graph_test
        logger.info("--- Test graph --- : graph {} Test Acc {} Kendall {} ---".format(graph_name, accuracy_graph_test, kendall_tau_graph_test))

    test_accuracy = rank_equal_num_test / rank_all_num_test
    test_kendall = (rank_equal_num_test - rank_unequal_num_test) / rank_all_num_test

    logger.info("--- Valid {} Test Acc {} Test Kendall {} ---".format(valid_loss / valid_loss_cnt, test_accuracy, test_kendall))

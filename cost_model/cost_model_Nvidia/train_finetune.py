...
# Configure logging
# logging.basicConfig(filename=f'{PROJECT_ROOT}/cost_model/cost_model_GPU/finetune_result/trainlog_gcn_transformer_algo.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
logger = get_logger("train_logger", f"{PROJECT_ROOT}/cost_model/cost_model_GPU/finetune_result/trainlog_gcn_transformer_algo.log")
...
def train_algo(algo_name='pagerank'):
    ...
    train_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/finetune/train/{algo_name}.csv'
    val_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/finetune/val/{algo_name}.csv'
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/finetune_result/costmodel_best.pth")
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/finetune_result/costmodel_{epoch+300}.pth")
...
def train_all_algo_v2():
    ...
    for epoch in range(epoches):
        ...
        for algo in ["pagerank", "sssp", "bfs", "cc",]:
            ...
            train_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/finetune/train/{algo}.csv'
            val_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/finetune/val/{algo}.csv'
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/finetune_result/costmodel_latest.pth")

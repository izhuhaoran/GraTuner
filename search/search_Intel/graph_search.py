...
def search(knn_graph, k, graph_id, algo_name, n, schedules_origin, schedules_data, model):
    ...
    logger = get_logger(f"search_{algo_name}_{graph_name}", f"{PROJECT_ROOT}/search/search_Intel/search_{algo_name}_{graph_name}.log")
...
def search_v2(knn_graph, k, graph_id, algo_name, n, schedules_origin, schedules_data, model):
    graph_name = graph_list[graph_id]
    logger = get_logger(f"search_v2_{algo_name}_{graph_name}", f"{PROJECT_ROOT}/search/search_Intel/search_v2_{algo_name}_{graph_name}.log")
...
@torch.no_grad()
def search_v2_print_iter(knn_graph, k, graph_name, algo_name, n, schedules_origin, schedules_data, model):
    logger = get_logger(f"search_v2_print_iter_{algo_name}_{graph_name}_{k}", f"{PROJECT_ROOT}/search/search_Intel/search_diff_k_iter_{algo_name}_{graph_name}_k{k}.log")
...
@torch.no_grad()
def search_v3_print_iter(knn_graph, k, graph_name, algo_name, n, schedules_origin, schedules_data, model, top_m=1):
    # logger = get_logger(f"search_v3_print_iter_{algo_name}_{graph_name}_{top_m}", f"{PROJECT_ROOT}/search/search_Intel/search_diff_topm_iter_{algo_name}_{graph_name}_top_m{top_m}.log")
    logger = get_logger(f"search_v3_print_iter_{algo_name}_{graph_name}_{top_m}", f"{PROJECT_ROOT}/search/search_Intel/search_tune_{algo_name}_{graph_name}.log")
    logger.addHandler(console_handler)
    
    t0 = time.perf_counter()
    g = create_dgl_graph(graph_name).to(device)
    t1 = time.perf_counter()
    logger.info(f"Create graph time: {t1 - t0} s")

    # 准备图特征
    graph_feature = model.embed_sparse_matrix(g)  # [1, out_dim=128]
    t2 = time.perf_counter()
    logger.info(f"Embed graph time: {t2 - t1} s")
    ...
    algo_feature = model.embed_algo(algo_op)  # [1, 128]
    t3 = time.perf_counter()
    logger.info(f"Embed algo time: {t3 - t2} s")
    ...
    t4 = time.perf_counter()
    logger.info(f"Initialization active set time: {t4 - t3} s")
    ...
    iter = 0
    cur_runtime_result = get_real_runtime_for_result(result_queue)
    logger.info(f"iter {iter}, Top k runtime: {cur_runtime_result} s, active_set: {active_set}")
    ...
    while active_set:
        ...
        iter += 1
        cur_runtime_result = get_real_runtime_for_result(result_queue)
        logger.info(f"iter {iter}, iter time: {t_iter_end - t_iter_start} s, Top k runtime: {cur_runtime_result}, active_set: {active_set}")
        if not active_set:
            break
    
    t5 = time.perf_counter()
    logger.info(f"Traversal search time: {t5 - t4} s")
    ...
    final_result = get_real_runtime_for_result(result_queue)
    t6 = time.perf_counter()
    logger.info(f"Final result time: {t6 - t5} s")
    logger.info(f"All tune time {t6 - t0} s")
    logger.info(f"Top k nodes for {algo_name} on {graph_name}: {final_result}")
    return final_result
...
def test_all_data_algo():
    model = AutoGraphModel()
    checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_Intel/finetune_result/costmodel_best.pth", map_location=device)
    model.load_state_dict(checkpoint)
    model = model.to(device)

    for _ in range(5):
        for algo_name in algo_list:
            schedules_origin, schedules_data = read_all_schedules(f"{PROJECT_ROOT}/search/search_Intel/all_true_schedule_{algo_name}.csv")
            n = len(schedules_data)
            for graph_id in range(len(graph_list)):
                knn_graph_path = f"{PROJECT_ROOT}/search/search_Intel/trained_graph/one_graph/knn_graph_best_{algo_name}_{graph_list[graph_id]}.pkl"
                ...
                for k in range(1, 11):
                    ...
                    result = search_v3_print_iter(knn_graph, 10, graph_id, algo_name, n, schedules_origin, schedules_data, model, top_m=k)
                    print(f"Top k nodes with highest scores for {algo_name} on {graph_list[graph_id]}: {result}")
                    # exit()

def tune(algo_name, graph_name, k=10, m=1):
    assert algo_name in algo_list, f"Invalid algorithm name: {algo_name}. Must be one of {algo_list}."
    assert graph_name in graph_list, f"Invalid graph name: {graph_name}. Must be one of {graph_list}."
    assert k > 0 and k <= 10, f"Invalid k value: {k}. Must be between 1 and 10."
    assert m > 0 and m <= 10, f"Invalid m value: {m}. Must be between 1 and 10."
    print("-" * 30 + f"Tune for {algo_name} on {graph_name}, k={k}, m={m}" + "-" * 30)

    schedules_origin, schedules_data = read_all_schedules(f"{PROJECT_ROOT}/search/search_Intel/all_true_schedule_{algo_name}.csv")
    n = len(schedules_data)

    knn_graph_path = f"{PROJECT_ROOT}/search/search_Intel/trained_graph/one_graph/knn_graph_best_{algo_name}_{graph_name}.pkl"
    knn_graph = load_knn_graph(knn_graph_path)
    ...
    result = search_v3_print_iter(knn_graph, k, graph_name, algo_name, n, schedules_origin, schedules_data, model, m)
    # print(f"Top k nodes with highest scores for {algo_name} on {graph_name}: {result}")
    print(
        "-" * 30 + "Tune Over" + "-" * 30 + "\n"
        f"best program runtime is: {result[0][0]} s\n"
        f"best schedule config is: {schedules_origin[result[0][1]]}\n"
    )

if __name__ == "__main__":
    argparser = argparse.ArgumentParser(description="Search for the best schedule")
    argparser.add_argument("--algo", type=str, default="bfs", help="Algorithm name")
    argparser.add_argument("--graph", type=str, default="youtube-u-growth", help="Graph name")
    argparser.add_argument("--k", type=int, default=10, help="Top k nodes to search")
    argparser.add_argument("--m", type=int, default=1, help="Top m neighbors to consider")

...
from cost_model.cost_model_Nvidia.model import AutoGraphModel
...
output_dir = f'{PROJECT_ROOT}/search/search_GPU/trained_graph'
...
def read_runtime_dict():
    ...
    for algo in ["bfs", "pagerank", "sssp", 'cc']:
        file_path = f"{PROJECT_ROOT}/dataset_GPU/finetune/train/{algo}.csv"
...
def q_learning_train(knn_graph, algo_name, graphid, model=None):
    # Configure logging
    # logging.basicConfig(filename=f'{output_dir}/q_learning_train.log', level=logger.info, format='%(asctime)s %(message)s')
    logger = get_logger(f'q_learning_train_{algo_name}_{graph_list[graphid]}', f'{output_dir}/q_learning_train_{algo_name}_{graph_list[graphid]}.log')
    logger.info(f"KNN graph for {algo_name} {graph_list[graphid]} start training...")

    schedules_origin, schedules_data = read_all_schedules(f"{PROJECT_ROOT}/search/search_GPU/all_true_schedule_{algo_name}_GPU.csv")
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            # logger.info(f"Episode {episode}, Epsilon: {epsilon}, Step {step}, Current state: {current_state}, Next state: {next_state}, Reward: {reward}")
            current_state = next_state

        search_result = evaluate_episode_by_search(knn_graph, 10, graphid, algo_name, schedules_origin, schedules_data, model)
        logger.info(f"Episode {episode}, Search result: {search_result}")
        if search_result > best_search_result:
            ...
            logger.info(f"Best KNN graph for {algo_name} on {graphname} saved at episode {best_episode}, evale search result: {search_result}")
...
def q_learning_train_all_graph(knn_graph, algo_name, model=None):
    # Configure logging
    # logging.basicConfig(filename=f'{output_dir}/q_learning_train_all_graph.log', level=logger.info, format='%(asctime)s %(message)s')
    logger = get_logger(f'q_learning_train_all_graph_{algo_name}', f'{output_dir}/q_learning_train_all_graph_{algo_name}.log')
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            for graphid, graphname in enumerate(graph_list):
                ...
                logger.info(f"Episode {episode}, Step {step}, Current state: {current_state}, Next state: {next_state}, Reward: {reward}, Epsilon: {epsilon}")
            current_state = next_state
        ...
        for graphid, graphname in enumerate(graph_list):
            ...
            if search_result > best_search_result[graphid]:
                ...
                logger.info(f"Best KNN graph for {algo_name} on {graphname} saved at episode {episode}, evale search result: {search_result}")

        # if (episode + 1) % 100 == 0:
        #     save_knn_graph(knn_graph, f"{output_dir}/all_graph/knn_graph_train_{algo_name}_{episode}.pkl")
        #     logger.info(f"KNN graph for {algo_name} saved at episode {episode}")
...
def q_learning_train_all_path(knn_graph, algo_name, graphid, model=None):
    ...
    # Configure logging
    # logging.basicConfig(filename=f'{output_dir}/q_learning_train_all_path.log', level=logger.info, format='%(asctime)s %(message)s')
    logger = get_logger(f'q_learning_train_all_path_{algo_name}_{graph_list[graphid]}', f'{output_dir}/q_learning_train_all_path_{algo_name}_{graph_list[graphid]}.log')
    logger.info(f"KNN graph for {algo} {graph_list[graphid]} start training...")
    ...
    for episode in range(num_episodes):
        ...
        # 评估和保存
        search_result = evaluate_episode_by_search(knn_graph, 10, graphid, algo_name, schedules_origin, schedules_data, model)
        logger.info(f"Episode {episode}, algo: {algo_name} graph: {graphname} Search result: {search_result}")
...
def q_learning_train_all_path_all_graph(knn_graph, algo_name, model=None):
    ...
    # Configure logging
    # logging.basicConfig(filename=f'{output_dir}/q_learning_train_all_path_all_graph.log', level=logger.info, format='%(asctime)s %(message)s')
    logger = get_logger(f'q_learning_train_all_path_all_graph_{algo_name}', f'{output_dir}/q_learning_train_all_path_all_graph_{algo_name}.log')
    ...
    for episode in range(num_episodes):
        ...
        for graphid, graphname in enumerate(graph_list):
            search_result = evaluate_episode_by_search(knn_graph, 10, graphid, algo_name, schedules_origin, schedules_data, model)
            logger.info(f"Episode {episode}, graph: {graphname} Search result: {search_result}")
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_GPU/finetune_result/costmodel_best.pth", map_location=device)
    model.load_state_dict(checkpoint)
    model = model.to(device)

    for algo in algo_list:
        for graphid in range(graph_num):
            knn_graph = load_knn_graph(f"{PROJECT_ROOT}/search/search_GPU/init_graph/knn_graph_init_{algo}.pkl")

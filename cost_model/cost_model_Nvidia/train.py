...
from .data_loader import ScheduleDataset_v2
from .model import AutoGraphModel, AutoGraphModel_algo
import logging
from common.common import PROJECT_ROOT

# Configure logging
# logging.basicConfig(filename=f'{PROJECT_ROOT}/cost_model/cost_model_GPU/train_result/trainlog_gcn_transformer.log', level=logging.INFO, format='%(asctime)s %(message)s')
...
logger = get_logger("train_logger", f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_result/trainlog_gcn_transformer.log")
...
def load_data(file_path, graph_col_id=0):
    ...
    origin_data = pd.read_csv(file_path, comment='#', sep=',', header=None, dtype=str, keep_default_na=False)
...
def train_onehot():
    algo_name = 'pagerank'
    train_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/train/{algo_name}.csv'
    val_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/val/{algo_name}.csv'
    ...
    # checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_result/costmodel_290.pth")
    ...
    for epoch in range(epoches):
        ...
        if test_accuracy > test_accuracy_best:
            torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_result/costmodel_best.pth")
        ...
        torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_result/costmodel_{epoch+300}.pth")
...
def train_all_algo_only_data():
    ...
    # checkpoint = torch.load(f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_result/costmodel_best_transformer.pth")
    # model.load_state_dict(checkpoint)
    ...
    for epoch in range(epoches):
        ...
        for algo in ["pagerank", "sssp", "bfs", "cc",]:
            ...
            train_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/train/{algo}.csv'
            val_file_path = f'{PROJECT_ROOT}/dataset/dataset_GPU/val/{algo}.csv'
        ...
        # torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_result/costmodel_{epoch}.pth")
...
#         if test_accuracy > test_accuracy_best:
#             torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_with_algo/costmodel_best.pth")
#             test_accuracy_best = test_accuracy
#             test_best_epoch = epoch
#             logger.info("--- Best Model Updated --- Epoch {} Test Acc {} ---".format(test_best_epoch, test_accuracy_best))
...
#         torch.save(model.state_dict(), f"{PROJECT_ROOT}/cost_model/cost_model_GPU/train_with_algo/costmodel_{epoch}.pth")

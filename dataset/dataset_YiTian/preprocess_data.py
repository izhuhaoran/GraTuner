...
def cat_all_algo_data():
    ...
    for algo in ["bfs", "pagerank", "sssp", 'cc']:
        input_path = f"{PROJECT_ROOT}/dataset_YiTian/train/{algo}.csv"
        with open(input_path, 'r') as f_in:
            for line in f_in:
                csv_line = f"{algo},{line}"
                all_lines.append(csv_line)

    with open("{PROJECT_ROOT}/dataset_YiTian/train/all_algo.csv", 'w') as f_out:
        for line in all_lines:
            f_out.write(line)
...
for algo in ["bfs", "pagerank", "sssp", 'cc']:
    input_path = f"{PROJECT_ROOT}/dataset_YiTian/finetune/{algo}.gt_output.txt"
    output_path_train = f"{PROJECT_ROOT}/dataset_YiTian/finetune_new/train/{algo}.csv"
    output_path_val = f"{PROJECT_ROOT}/dataset_YiTian/finetune_new/val/{algo}.csv"
